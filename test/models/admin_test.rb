...
class AdminTest < ActiveSupport::TestCase
  test "Has an email - database" do
    admin = Admin.new(name: 'Joe', password: '123')
  ...
  test "Has a name - database" do
    admin = Admin.new(email: 'joe@adobe.com', password: '123')
    assert_raises(ActiveRecord::NotNullViolation) do
      admin.save!(validate: false)
    end
  end
  ...
  test 'Emails are unique - database' do
    admin = Admin.create!(name: 'joe', email: 'joe@adobe.com', password: '123')
    duplicate = Admin.new(name: 'joe', email: 'joe@adobe.com', password: '123')
